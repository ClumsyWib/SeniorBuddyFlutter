...
class _MyCaretakerScreenState extends State<MyCaretakerScreen> {
  ...
  Future<void> _loadCaretaker() async {
    ...
    if (mounted) {
      if (result['success'] == true && result['data'] != null) {
        ...
        setState(() {
          _isLoading = false;
          _errorMessage = result['error']?.toString() ?? 'No caretaker assigned yet.';
        });
  ...
  Future<void> _loadAvailableCaretakers() async {
    ...
    if (mounted) {
      setState(() {
        ...
        _availableCaretakers = result['success'] == true ? (result['data'] as List<dynamic>? ?? []) : [];
      });
    }
  }
  ...
  Future<void> _showAssignCaretakerDialog() async {
    ...
    final List<dynamic> caretakers = result['success'] == true ? (result['data'] as List<dynamic>? ?? []) : [];

    if (caretakers.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No available caretakers found.'), backgroundColor: Colors.orange),
      );
      return;
    }
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Assign Caretaker', style: TextStyle(fontWeight: FontWeight.bold)),
        content: SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (_, i) {
              ...
              return ListTile(
                ...
                title: Text('${ct['first_name'] ?? ''} ${ct['last_name'] ?? ''}'.trim()),
                subtitle: Text(ct['username']?.toString() ?? ''),
                onTap: () async {
                  Navigator.pop(ctx);
                  if (widget.seniorId == null) return;

                  // Safely parse ID
                  final int caretakerId = int.tryParse(ct['id']?.toString() ?? '0') ?? 0;
                  ...
                  if (assignResult['success'] == true) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Caretaker assigned successfully!'), backgroundColor: Colors.green),
                    );
                    _loadCaretaker();
                  } else {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text('Failed: ${assignResult['error']}'), backgroundColor: Colors.red),
                    );
                  }
                },
              );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Caretaker Assignment', style: TextStyle(fontSize: 24)),
                ...
                child: Column(
                  children: [
                    CircleAvatar(
                      ...
                      child: const Icon(Icons.person, size: 70, color: Color(0xFF2196F3)),
                    ),
                    ...
                    const SizedBox(height: 24),
                    _buildInfoRow(Icons.work, 'Experience', _caretaker!['experience']?.toString() ?? 'N/A'),
                    const SizedBox(height: 12),
                    _buildInfoRow(Icons.schedule, 'Schedule', _caretaker!['schedule']?.toString() ?? 'Flexible'),
                    const SizedBox(height: 24),
                    const Text(
            ...
            children: [
              Expanded(
                child: ElevatedButton.icon(
                  onPressed: () => _makePhoneCall(_caretaker?['phone']?.toString()),
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            const Text('No caretakers available.', style: TextStyle(fontSize: 20, color: Colors.black54)),
            const SizedBox(height: 24),
            ElevatedButton(onPressed: _loadAvailableCaretakers, child: const Text('Refresh List')),
          ],
        ),
      );
    }
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final fullName = '${ct['first_name'] ?? ''} ${ct['last_name'] ?? ''}'.trim();
              return Card(
                elevation: 2,
                ...
                child: ListTile(
                  ...
                  title: Text(fullName.isEmpty ? (ct['username']?.toString() ?? 'Caretaker') : fullName,
                      style: const TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text(ct['specialization']?.toString() ?? 'Caregiver'),
                  trailing: ElevatedButton(
                    onPressed: () {
                      final caretakerId = int.tryParse(ct['id']?.toString() ?? '0') ?? 0;
