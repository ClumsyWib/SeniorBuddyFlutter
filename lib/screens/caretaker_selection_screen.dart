...
class CaretakerSelectionScreen extends StatefulWidget {
  final int seniorId;
  const CaretakerSelectionScreen({Key? key, required this.seniorId}) : super(key: key);
...
class _CaretakerSelectionScreenState extends State<CaretakerSelectionScreen> {
    ...
    if (mounted) {
      setState(() {
        ...
        if (result['success']) {
          ...
          _errorMessage = result['error']?.toString() ?? 'Failed to load caretakers';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Select Caretaker', style: TextStyle(fontWeight: FontWeight.bold)),
      ...
      body: _isLoading
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            const Text('No caretakers available', style: TextStyle(fontSize: 18, color: Colors.grey)),
          ],
        ),
      )
          : ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final user = caretaker['user'] ?? {}; // Use 'user' instead of 'user_details'
          ...
          return Card(
            ...
            child: ListTile(
              ...
              leading: CircleAvatar(
                ...
                child: photo == null ? const Icon(Icons.person, size: 30, color: Colors.teal) : null,
              ),
              title: Text(
                name,
              ...
              subtitle: Column(
                ...
                children: [
                  ...
                  Text(caretaker['specialization'] ?? 'Professional Caretaker'),
                  const SizedBox(height: 4),
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 4),
                      Text('${caretaker['experience_years'] ?? '0'} Years'),
                    ],
                  ),
                ],
              ),
