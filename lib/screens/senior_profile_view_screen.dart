...
class SeniorProfileViewScreen extends StatefulWidget {
  ...
  @override
  State<SeniorProfileViewScreen> createState() =>
      _SeniorProfileViewScreenState();
}
...
class _SeniorProfileViewScreenState extends State<SeniorProfileViewScreen> {
                  ...
                  child: Column(
                    children: [
                      ...
                      _buildInfoCard(
                        ...
                        items: [
                          _buildLargeInfoRow(
                              'Age', '${_seniorData?['age'] ?? '-'} years'),
                          _buildLargeInfoRow(
                              'Gender', _seniorData?['gender'] ?? 'Unknown'),
                          _buildLargeInfoRow(
                              'City', _seniorData?['city'] ?? 'Not set'),
                          _buildLargeInfoRow(
                              'Address', _seniorData?['address'] ?? 'Not set'),
                        ],
                      ),
                      const SizedBox(height: 24),
                      ...
                      _buildInfoCard(
                        ...
                        items: [
                          _buildLargeInfoRow('Medical Info',
                              _seniorData?['medical_info'] ?? 'No info'),
                          _buildLargeInfoRow(
                              'Emergency',
                              _seniorData?['emergency_contact'] ??
                                  'No contact'),
                        ],
                      ),
                      const SizedBox(height: 40),
  ...
  Widget _buildInfoCard({
    ...
    final effectiveColor = color ?? Theme.of(context).primaryColor.withOpacity(0.08);
    final effectiveTitleColor = titleColor ?? Theme.of(context).primaryColor;
    return Card(
      elevation: 4,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(icon, color: effectiveTitleColor, size: 32),
