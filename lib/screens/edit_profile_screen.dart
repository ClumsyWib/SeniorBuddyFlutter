...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  void initState() {
    ...
    _zipController = TextEditingController(text: widget.userData['zip_code'] ?? '');
    _dobController = TextEditingController(text: widget.userData['date_of_birth'] ?? '');
    ...
    try {
      ...
      if (_userRole == kRoleCaretaker) {
        ...
        if (result['success']) {
          ...
          _caretakerExperienceController.text = _roleProfileData?['experience_years']?.toString() ?? '0';
          _caretakerSkillsController.text = _roleProfileData?['skills'] ?? '';
          _caretakerRateController.text = _roleProfileData?['hourly_rate']?.toString() ?? '0.00';
          _caretakerBioController.text = _roleProfileData?['bio'] ?? '';
          _caretakerSpecializationController.text = _roleProfileData?['specialization'] ?? '';
          _caretakerAvailabilityStatus = _roleProfileData?['availability_status'] ?? 'Full-time';
        ...
        if (seniorId != null) {
          ...
          if (result['success']) {
            ...
            _seniorAgeController.text = _roleProfileData?['age']?.toString() ?? '';
            _seniorGender = _roleProfileData?['gender'] ?? 'Male';
            _seniorMedicalController.text = _roleProfileData?['medical_conditions'] ?? '';
            _seniorAllergiesController.text = _roleProfileData?['allergies'] ?? '';
            _seniorRoutineController.text = _roleProfileData?['daily_routine'] ?? '';
            _seniorMobilityController.text = _roleProfileData?['mobility_status'] ?? '';
            _seniorCareLevelController.text = _roleProfileData?['care_level'] ?? '';
            _seniorDoctorController.text = _roleProfileData?['primary_doctor'] ?? '';
            _seniorDoctorPhoneController.text = _roleProfileData?['doctor_phone'] ?? '';
          }
        }
        ...
        if (result['success']) {
          ...
          _volunteerSpecializationController.text = _roleProfileData?['specialization'] ?? '';
          _volunteerAvailabilityStatus = _roleProfileData?['availability_status'] ?? 'Part-time';
    ...
    try {
      ...
      if (_userRole == kRoleCaretaker) {
        final cResult = await _apiService.updateCaretakerProfile({
          'experience_years': int.tryParse(_caretakerExperienceController.text) ?? 0,
        ...
        if (!cResult['success']) {
          throw Exception(cResult['error'] ?? 'Failed to update caretaker details');
        }
      } else if (_userRole == kRoleVolunteer) {
        ...
        if (!vResult['success']) {
          throw Exception(vResult['error'] ?? 'Failed to update volunteer details');
        }
        ...
        final sResult = await _apiService.updateSeniorProfile(seniorId, {
          'name': _firstNameController.text.trim(), // Senior name comes from the 'Full Name' field we mapped to _firstNameController
        ...
        if (!sResult['success']) {
          throw Exception(sResult['error'] ?? 'Failed to update senior details');
        }
      }
      ...
      if (_imageFile != null) {
        if (_userRole != kRoleSenior) {
          final photoResult = await _apiService.updateProfilePicture(_imageFile!.path);
  ...
  Widget build(BuildContext context) {
    final primaryColor = _getRoleColor();
            ...
            children: [
                  ...
                  children: [
                    CircleAvatar(
                      ...
                      backgroundImage: _imageFile != null
                          ...
                          ? NetworkImage(widget.userData['profile_picture']) as ImageProvider
                          : null),
              ...
              if (_userRole != kRoleSenior) ...[
                ...
                InkWell(
                  ...
                  child: IgnorePointer(
                    child: _buildTextField(_dobController, 'Date of Birth', Icons.cake_outlined),
                  ),
                ),
                const SizedBox(height: 32),
              ],
              ...
              if (_userRole == kRoleCaretaker) ...[
                ...
                const SizedBox(height: 16),
                _buildTextField(_caretakerSpecializationController, 'Specialization', Icons.workspace_premium_outlined),
                const SizedBox(height: 16),
                _buildTextField(_caretakerExperienceController, 'Experience (Years)', Icons.history, keyboardType: TextInputType.number),
                const SizedBox(height: 16),
                _buildTextField(_caretakerRateController, 'Hourly Rate ()', Icons.payments_outlined, keyboardType: TextInputType.number),
                const SizedBox(height: 16),
                _buildTextField(_caretakerSkillsController, 'Skills', Icons.psychology_outlined),
                const SizedBox(height: 16),
                _buildTextField(_caretakerBioController, 'Biography', Icons.description_outlined, maxLines: 4),
                const SizedBox(height: 16),
                DropdownButtonFormField<String>(
                  value: _caretakerAvailabilityStatus,
                  decoration: InputDecoration(
                    labelText: 'Availability Status',
                    prefixIcon: const Icon(Icons.event_available, color: Colors.grey),
                  ...
                  onChanged: (val) => setState(() => _caretakerAvailabilityStatus = val!),
                ),
                const SizedBox(height: 8),
                SwitchListTile(
                  title: const Text('Available for Work', style: TextStyle(fontWeight: FontWeight.w500)),
                  subtitle: const Text('Toggle your working status'),
                  value: _caretakerIsAvailable,
                  activeColor: primaryColor,
                  onChanged: (val) => setState(() => _caretakerIsAvailable = val),
                ),
                const SizedBox(height: 32),
              ],
              ...
              if (_userRole == kRoleVolunteer) ...[
                ...
                const SizedBox(height: 16),
                _buildTextField(_volunteerSpecializationController, 'Specialization/Role', Icons.workspace_premium_outlined),
                const SizedBox(height: 16),
                _buildTextField(_volunteerSkillsController, 'Skills', Icons.psychology_outlined),
                const SizedBox(height: 16),
                _buildTextField(_volunteerBioController, 'Biography', Icons.description_outlined, maxLines: 4),
                const SizedBox(height: 16),
                DropdownButtonFormField<String>(
                  ...
                  items: ['Full-time', 'Part-time', 'Flexible'].map((s) => DropdownMenuItem(value: s, child: Text(s))).toList(),
                  onChanged: (val) => setState(() => _volunteerAvailabilityStatus = val!),
                ),
                const SizedBox(height: 8),
                SwitchListTile(
                  title: const Text('Active Volunteer', style: TextStyle(fontWeight: FontWeight.w500)),
                  subtitle: const Text('Toggle availability'),
                  value: _volunteerIsAvailable,
                  activeColor: primaryColor,
                  onChanged: (val) => setState(() => _volunteerIsAvailable = val),
                ),
                const SizedBox(height: 32),
              ],
              ...
              if (_userRole == kRoleSenior) ...[
                ...
                const SizedBox(height: 16),
                _buildTextField(_firstNameController, 'Full Name', Icons.person_outline), // Using firstName field for Senior name
                const SizedBox(height: 16),
                Row(
                  children: [
                    Expanded(child: _buildTextField(_seniorAgeController, 'Age', Icons.calendar_today, keyboardType: TextInputType.number)),
                    ...
                    Expanded(
                      child: DropdownButtonFormField<String>(
                        ...
                        onChanged: (val) => setState(() => _seniorGender = val!),
                      ),
                    ),
                  ],
                ),
                const SizedBox(height: 16),
                _buildTextField(_seniorMedicalController, 'Medical Conditions', Icons.medical_services_outlined, maxLines: 2),
                const SizedBox(height: 16),
                _buildTextField(_seniorAllergiesController, 'Allergies', Icons.warning_amber_outlined),
                const SizedBox(height: 16),
                _buildTextField(_seniorRoutineController, 'Daily Routine', Icons.schedule_outlined, maxLines: 2),
                const SizedBox(height: 16),
                _buildTextField(_seniorMobilityController, 'Mobility Status', Icons.directions_walk_outlined),
                const SizedBox(height: 16),
                _buildTextField(_seniorCareLevelController, 'Care Level', Icons.health_and_safety_outlined),
                const SizedBox(height: 16),
                _buildTextField(_seniorDoctorController, 'Primary Doctor', Icons.person_add_alt_1_outlined),
                const SizedBox(height: 16),
                _buildTextField(_seniorDoctorPhoneController, 'Doctor Phone', Icons.phone_callback_outlined, keyboardType: TextInputType.phone),
                const SizedBox(height: 32),
              ],
              ...
              if (_userRole != kRoleSenior) ...[
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 16),
                    Expanded(child: _buildTextField(_zipController, 'Zip Code', Icons.pin_drop_outlined, keyboardType: TextInputType.number)),
                  ],
                ),
                const SizedBox(height: 32),
                ...
                const SizedBox(height: 16),
                _buildTextField(_emergencyNameController, 'Contact Name', Icons.contact_emergency_outlined),
                const SizedBox(height: 16),
                _buildTextField(_emergencyPhoneController, 'Contact Phone', Icons.phone_callback_outlined, keyboardType: TextInputType.phone),
                const SizedBox(height: 32),
              ],
  ...
  Color _getRoleColor() {
    switch (_userRole) {
      case kRoleFamily:    return const Color(0xFF43A047);
      case kRoleCaretaker: return const Color(0xFF1976D2);
      case kRoleSenior:    return const Color(0xFF8E24AA);
      case kRoleVolunteer: return const Color(0xFFF59E0B);
