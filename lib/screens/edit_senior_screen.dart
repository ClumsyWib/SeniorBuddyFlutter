...
class _EditSeniorScreenState extends State<EditSeniorScreen> {
  ...
  void initState() {
    ...
    _ageController =
        TextEditingController(text: widget.senior['age']?.toString() ?? '');

    // Normalize gender to match dropdown items (Male, Female, Other)
    String genderRaw =
        (widget.senior['gender'] ?? 'Male').toString().trim().toLowerCase();
    if (genderRaw == 'female') {
      _gender = 'Female';
    ...
    _medicalInfoController =
        TextEditingController(text: widget.senior['medical_info'] ?? '');
    _medicalConditionsController =
        TextEditingController(text: widget.senior['medical_conditions'] ?? '');
    _allergiesController =
        TextEditingController(text: widget.senior['allergies'] ?? '');
    _routineController =
        TextEditingController(text: widget.senior['daily_routine'] ?? '');
    _mobilityController =
        TextEditingController(text: widget.senior['mobility_status'] ?? '');
    _careLevelController =
        TextEditingController(text: widget.senior['care_level'] ?? '');
    _doctorController =
        TextEditingController(text: widget.senior['primary_doctor'] ?? '');
    _doctorPhoneController =
        TextEditingController(text: widget.senior['doctor_phone'] ?? '');
    _emergencyContactController =
        TextEditingController(text: widget.senior['emergency_contact'] ?? '');
    _addressController =
        TextEditingController(text: widget.senior['address'] ?? '');
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      final result = await _apiService.updateSeniorProfile(
          widget.senior['id'], data,
      ...
      if (result['success']) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text('Senior profile updated!'),
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                          ...
                          CircleAvatar(
                            ...
                            backgroundImage: _imageFile != null
                                ...
                                : (widget.senior['photo'] != null
                                    ? NetworkImage(widget.senior['photo'])
                                        as ImageProvider
                                    : null),
                    ...
                    const SizedBox(height: 16),
                    _buildTextField(_medicalInfoController, 'Health Summary',
                    ...
                    _buildTextField(_emergencyContactController,
                        'Emergency Contact Phone', Icons.emergency_outlined,
                    ...
                    const SizedBox(height: 16),
                    _buildTextField(
                        _addressController, 'Full Address', Icons.home_outlined,
                        maxLines: 2),
                    const SizedBox(height: 32),

                    _buildSectionTitle('Healthcare Providers'),
                    const SizedBox(height: 16),
                    _buildTextField(_doctorController, 'Primary Doctor Name',
                        Icons.person_add_alt_1_outlined),
                    const SizedBox(height: 16),
                    _buildTextField(_doctorPhoneController,
                        'Doctor Phone Number', Icons.phone_callback_outlined,
                        keyboardType: TextInputType.phone),
                    const SizedBox(height: 32),

                    _buildSectionTitle('Daily Care'),
                    const SizedBox(height: 16),
                    _buildTextField(_routineController, 'Daily Routine',
                        Icons.schedule_outlined,
                        maxLines: 2),
                    const SizedBox(height: 16),
                    _buildTextField(_mobilityController, 'Mobility Status',
                        Icons.directions_walk_outlined),
                    const SizedBox(height: 16),
                    _buildTextField(_careLevelController, 'Care Level',
