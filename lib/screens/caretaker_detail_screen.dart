...
class _CaretakerDetailScreenState extends State<CaretakerDetailScreen> {
  ...
  Future<void> _handleAssign() async {
    ...
    final user = widget.caretaker['user'] ?? {}; // Use 'user' instead of 'user_details'
    ...
    if (mounted) {
      ...
      if (result['success']) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to assign: ${result['error']}'), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Caretaker Profile', style: TextStyle(fontWeight: FontWeight.bold)),
                  ...
                  child: Column(
                    children: [
                      CircleAvatar(
                        ...
                        child: photo == null ? const Icon(Icons.person, size: 60, color: Colors.teal) : null,
                      ),
                      ...
                      Text(
                        widget.caretaker['specialization'] ?? 'Professional Caretaker',
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      _buildStatColumn('Rating', widget.caretaker['rating']?.toString() ?? '0.0', Icons.star, Colors.amber),
                      _buildStatColumn('Experience', '${widget.caretaker['experience_years'] ?? 0} Years', Icons.work, Colors.blue),
                      _buildStatColumn('Availability', widget.caretaker['availability_status'] ?? 'Full-time', Icons.event_available, Colors.green),
                    ],
                  ),
