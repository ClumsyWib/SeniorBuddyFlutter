...
class ProfileHeader extends StatelessWidget {
  ...
  List<Widget> _buildStats(BuildContext context) {
    if (userRole == kRoleCaretaker) {
      final rating =
          userData?['caretaker_profile']?['rating']?.toString() ?? '0.00';
      final exp =
          '${userData?['caretaker_profile']?['experience_years'] ?? 0}+ Years';
      final avail =
          userData?['caretaker_profile']?['availability_status'] ?? 'Full-time';

      return [
        _buildStatItem('Rating', rating, Icons.star, Colors.amber),
        _buildStatItem('Experience', exp, Icons.work, Colors.blue),
        _buildStatItem(
            'Availability', avail, Icons.event_available, Colors.green),
      ];
    } else if (userRole == kRoleVolunteer) {
      final rating =
          userData?['volunteer_profile']?['rating']?.toString() ?? '0.00';
      final exp =
          userData?['volunteer_profile']?['availability_status'] ?? 'Part-time';
      final hours =
          '${userData?['volunteer_profile']?['total_hours']?.toInt() ?? 0} Hrs';
      ...
      return [
        ...
        _buildStatItem(
            'Care Level', level, Icons.health_and_safety, Colors.teal),
      ];
    }
    ...
    return [
      ...
      _buildStatItem('City', userData?['city'] ?? 'My City', Icons.location_on,
          Colors.blue),
    ];
  }
